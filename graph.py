...
class Vertex(object):
    ...
    @property
    def set_of_neighbours(self) -> Set:
        return set(self._incidence.keys())
...
class Graph(object):
    ...
    def remove_vertex(self, vertex: "Vertex"):
        ...
        if vertex.degree >0:
            poff=set()
            for value in vertex._incidence.values():
                poff.add(next(iter(value)))
            for edge in poff:
                temp=edge
                temp.head._remove_incidence(temp)
                temp.tail._remove_incidence(temp)
                if edge in self._e:
                    self._e.remove(temp)
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        new = Graph(self._directed)
        new._v = self._v + other._v
        new._e = self._e + other._e
        i = 0
        for v in new.vertices:
            if i < len(self):
                v.oldmatch = self.vertices[i]
            else:
                v.oldmatch = other.vertices[i - len(self)]
            i = i + 1
        return new
